...
type Theme = 'original' | 'tailwind dark' | 'black & white' | 'white & black' | 'cyberpunk';

let selectedTheme: Theme = (localStorage.getItem('theme') as Theme) ?? 'original';
let bestScore = 0;
let score = 0;
interface Tile {
  id: string,
...
function isDark(hex: string): boolean {
  const rgb = hex.toLowerCase().replace('#', '').match(/.{2}/g)?.map((x) => parseInt(x, 16));
...
function changeTheme(theme: Theme) {
  ...
  document.querySelector<HTMLDivElement>('.board-container')!.style.backgroundColor = themes[selectedTheme].color1;
  document.querySelectorAll<HTMLDivElement>('.board-grid').forEach((grid: HTMLDivElement) => {
    grid.style.backgroundColor = themes[selectedTheme].color2;
  });

  board.forEach((tile) => {
    document.getElementById(tile.id)!.querySelector('.inner-tile')!.setAttribute('style', `
      background-color: ${colorPalette[Math.log2(tile.value) - 1]};
      color: ${isDark(colorPalette[Math.log2(tile.value) - 1]) ? themes[selectedTheme].fg_light : themes[selectedTheme].fg_dark};
    `);
  });
  ...
  Array(4).fill(0).forEach((_, i) => {
    document.getElementById(`rect-${i + 1}`)!.setAttribute('fill', themes[selectedTheme].board[i]);
    document.getElementById(`box-text-${i}`)!.setAttribute('fill', isDark(themes[selectedTheme].board[i]) ? themes[selectedTheme].fg_light : themes[selectedTheme].fg_dark);
  });

  document.getElementById('newgame')!.setAttribute('style', `
...
function initBoard() {
  ...
  app.innerHTML = `
        ...
        <div class="dropdown dropdown-end">
          ...
          <div tabindex="0" class="dropdown-content translate-x-16 sm:translate-x-0 p-2 shadow rounded-box w-80 sm:w-96 flex flex-col gap-2">
            ${Object.entries(themes).map(([name, theme]) => `
              <button onclick="changeTheme('${name}')" id="theme-${name}" class="rounded-lg !flex flex-row items-center justify-between p-4 ${name === selectedTheme && 'selected'}" style="
    ...
    <div class="board-container w-[21rem] sm:w-[29rem] h-[21rem] sm:h-[29rem] flex items-center justify-center rounded-lg" style="background-color: ${themes[selectedTheme].color1}">
      <div id="board" class="w-[20rem] sm:w-[28rem] h-[20rem] sm:h-[28rem] relative grid grid-cols-4 overflow-hidden items-center justify-center">
        ${Array(16).fill(0).map(() => `<div class="w-[4.4rem] sm:w-24 h-[4.4rem] sm:h-24 board-grid rounded-md m-1 sm:m-2 border-yellow-400" style="background-color: ${themes[selectedTheme].color2}"></div>`).join('')}
      </div>
    </div>
    <p class="fixed w-full px-8 left-1/2 bottom-6 -translate-x-1/2 text-xs text-center">
...
function sortBoard(direction: 'up' | 'down' | 'left' | 'right'): Tile[] {
  if (direction === 'up' || direction === 'down') {
    board.sort((a, b) => {
      ...
      return direction === 'down' ? b.position.y - a.position.y : a.position.y - b.position.y;
    });
  } else {
    board.sort((a, b) => {
      ...
      return direction === 'right' ? b.position.x - a.position.x : a.position.x - b.position.x;
    });
  }
...
function createNewTile(tile: Tile, isGenerated: boolean): HTMLDivElement {
  ...
  newTileElement.classList.add('tile', isGenerated ? 'generated-tile' : 'new-tile', `tile-pos-${tile.position.x}-${tile.position.y}`);
...
function compress(direction: 'up' | 'down' | 'left' | 'right'): void {
  ...
  for (const tile of board) {
    if (tile.position[getYX(direction)] !== (['left', 'up'].includes(direction) ? 0 : 3)) {
      let i = ['left', 'up'].includes(direction) ? 0 : 3;
      let found = false;
      while (['left', 'up'].includes(direction) ? (
        i <= tile.position[getYX(direction)] + 1
      ) : (
        i >= tile.position[getYX(direction)] - 1
      )) {
        if (board.findIndex((t) => t.position[['x', 'y'][isLeftRight(direction)] as 'x' | 'y'] === tile.position[['x', 'y'][isLeftRight(direction)] as 'x' | 'y']
      ...
      if (found && (
        ...
        document.getElementById(tile.id)?.classList.add(`tile-pos-${['left', 'right'].includes(direction) ? i : tile.position.x}-${['left', 'right'].includes(direction) ? tile.position.y : i}`);
        ...
        for (const t of tile.mergeCells) {
          ...
          tl?.classList.add(`tile-pos-${['left', 'right'].includes(direction) ? i : tile.position.x}-${['left', 'right'].includes(direction) ? tile.position.y : i}`);
        }
      }
    }
  }
...
function merge(direction: 'up' | 'down' | 'left' | 'right'): void {
  ...
  for (const tile of board) {
    if ((['left', 'up'].includes(direction) ? tile.position[getYX(direction)] < 3 : tile.position[getYX(direction)] > 0) && !tile.isMerged) {
      const targetIndex = board.findIndex(
        (t) => t.position[getXY(direction)] === tile.position[getXY(direction)]
          && t.position[getYX(direction)] === tile.position[getYX(direction)] - (['left', 'up'].includes(direction) ? -1 : 1)
          && t.value === tile.value,
      );
      if (targetIndex !== -1) {
        ...
        targetDiv?.classList.add(`tile-pos-${target.position.x + (['left', 'right'].includes(direction) ? [1, -1][Number(direction === 'left')] : 0)}-${target.position.y + (['up', 'down'].includes(direction) ? [1, -1][Number(direction === 'up')] : 0)}`);
        target.position[getYX(direction)] += (['left', 'up'].includes(direction) ? -1 : 1);
...
function generateNewTile(value?: number): void {
  ...
  if (emptyTiles.length) {
    ...
    const newTile = {
      id: v4(),
      value: value ?? [2, 2, 2, 2, 2, 2, 2, 2, 2, 4][Math.floor(Math.random() * 10)],
      position: {
        x: randomTile.x,
        y: randomTile.y,
      },
...
const move = (direction: 'left' | 'right' | 'up' | 'down') => {
  ...
  const lastBoard = JSON.parse(
    JSON.stringify(sortBoard(direction)),
