...
function initBoard() {
  ...
  app.innerHTML = `
    ...
    <div class="w-[29rem] h-[29rem] flex items-center justify-center bg-slate-200 dark:bg-slate-900 rounded-lg">
      <div id="board" class="w-[28rem] h-[28rem] relative grid grid-cols-4 overflow-hidden items-center justify-center">
        ${Array(16).fill(0).map(() => '<div class="w-24 h-24 rounded-md m-2 bg-slate-100 dark:bg-slate-800 border-yellow-400"></div>').join('')}
      </div>
    </div>
...
function createNewTile(tile: Tile, isGenerated: boolean): HTMLDivElement {
  ...
  newTileElement.innerHTML = `<div class="inner-tile ${COLOR_PALETTE[Math.log2(tile.value)]} text-slate-100">${tile.value}</div>`;
