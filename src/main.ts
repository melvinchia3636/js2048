...
type Theme = 'original' | 'tailwind dark' | 'black & white';

let selectedTheme: Theme = 'original';
interface Tile {
  id: string,
  value: number;
  position: {
...
function initBoard() {
  ...
  app.innerHTML = `
    <nav class="w-full flex justify-between items-center p-8 fixed top-0 left-0">
      ...
      <div class="flex items-center">
        <div class="dropdown dropdown-end">
          ...
          <div tabindex="0" class="dropdown-content p-2 shadow rounded-box w-96 flex flex-col gap-2">
    ...
    <div class="board-container w-[29rem] h-[29rem] flex items-center justify-center rounded-lg" style="background-color: ${themes[selectedTheme].color1}">
      <div id="board" class="w-[28rem] h-[28rem] relative grid grid-cols-4 overflow-hidden items-center justify-center">
        ${Array(16).fill(0).map(() => `<div class="w-24 h-24 board-grid rounded-md m-2 border-yellow-400" style="background-color: ${themes[selectedTheme].color2}"></div>`).join('')}
      </div>
    </div>
    <p class="fixed left-1/2 bottom-6 -translate-x-1/2 text-xs text-center">
...
function generateNewTile(value?: number): void {
  ...
  if (emptyTiles.length) {
    ...
    const newTile = {
      id: v4(),
      value: value ?? [2, 2, 4][Math.floor(Math.random() * 3)],
      position: {
        x: randomTile.x,
        y: randomTile.y,
      },
